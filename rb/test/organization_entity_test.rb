...
class OrganizationEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = DataGovAuSDK.test(seed, nil)
    seen = base.Organization(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = DataGovAuConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = DataGovAuSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
      sdk.Organization(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DATA_GOV_AU_TEST_ORGANIZATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def organization_basic_setup(extra)
  ...
  client = DataGovAuSDK.test(options, extra)
  ...
  entid_env_raw = ENV["DATA_GOV_AU_TEST_ORGANIZATION_ENTID"]
  ...
  if env["DATA_GOV_AU_TEST_LIVE"] == "TRUE"
    ...
    client = DataGovAuSDK.new(Helpers.to_map(merged_opts))
  end
