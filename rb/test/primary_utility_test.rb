...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = DataGovAuOmni.make_runner(TEST_JSON_FILE, DataGovAuSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : DataGovAuOmni.errify(err)
    end
