...
module DataGovAuUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DataGovAuResponse.new({})
    ...
    if fetch_err
      ...
      response = DataGovAuResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DataGovAuResponse.new(fetched)
    else
