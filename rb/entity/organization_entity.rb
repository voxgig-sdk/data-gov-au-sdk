# DataGovAu SDK Organization entity
...
class OrganizationEntity
  ...
  def make
    opts = @_entopts.dup
    OrganizationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DataGovAuHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = DataGovAuHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Organization.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Organization, Hash] the loaded Organization; raises DataGovAuError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DataGovAuHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [OrganizationListMatch, Hash, nil] match filter (any subset of
  #   Organization fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Organization>, Array] the matching Organization items; raises DataGovAuError on failure
