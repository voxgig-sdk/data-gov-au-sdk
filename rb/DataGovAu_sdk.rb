...
require_relative 'DataGovAu_types'
...
class DataGovAuSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = DataGovAuHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DataGovAuHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, DataGovAuFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    DataGovAuUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = DataGovAuHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = DataGovAuHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = DataGovAuHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = DataGovAuHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DataGovAuError.new(
        "graphql_error", "DataGovAuSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
