...
The Python SDK for the DataGovAu API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Dataset(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.DataGovAu(None).load(
    {"id": "test01"}, None
)
