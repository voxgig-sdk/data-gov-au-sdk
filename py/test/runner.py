...
class DataGovAuTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DataGovAuTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DataGovAuTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DataGovAuTestRunner.getenv("DATA_GOV_AU_TEST_LIVE")
        override = DataGovAuTestRunner.getenv("DATA_GOV_AU_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DataGovAuTestRunner.getenv(key)
        ...
        explain = DataGovAuTestRunner.getenv("DATA_GOV_AU_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if DataGovAuTestRunner._test_control is not None:
            return DataGovAuTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DataGovAuTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DataGovAuTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DataGovAuTestRunner.load_test_control()
...
def load_env_local():
    DataGovAuTestRunner.load_env_local()


def env_override(m):
    return DataGovAuTestRunner.env_override(m)


def entity_data(v):
    return DataGovAuTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DataGovAuTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DataGovAuTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DataGovAuTestRunner.load_test_control()


def live_client_options():
    return DataGovAuTestRunner.live_client_options()


def live_delay_ms():
    return DataGovAuTestRunner.live_delay_ms()
