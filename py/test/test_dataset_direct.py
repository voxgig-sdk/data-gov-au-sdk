...
class TestDatasetDirect:

    def test_should_direct_load_dataset(self):
        setup = _dataset_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-dataset", "live" if setup["live"] else "unit")
        if _skip:
...
def _dataset_direct_setup(mockres):
