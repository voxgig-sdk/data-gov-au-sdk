...
class TestOrganizationEntity:
    ...
    def test_should_stream(self):
        ...
        base = DataGovAuSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DATA_GOV_AU_TEST_ORGANIZATION_ENTID JSON to run live")
        client = setup["client"]
...
def _organization_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "DATA_GOV_AU_TEST_ORGANIZATION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DATA_GOV_AU_TEST_ORGANIZATION_ENTID"))
    ...
    if env.get("DATA_GOV_AU_TEST_LIVE") == "TRUE":
        ...
        client = DataGovAuSDK(helpers.to_map(merged_opts))

    _live = env.get("DATA_GOV_AU_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("DATA_GOV_AU_TEST_EXPLAIN") == "TRUE",
        "live": _live,
