...
from datagovau_sdk.datagovau_types import (
    Dataset,
    DatasetLoadMatch,
)
...
class DatasetEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DatasetEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Dataset:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Dataset().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
