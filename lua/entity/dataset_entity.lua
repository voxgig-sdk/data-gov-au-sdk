...
local DatasetEntity = {}
DatasetEntity.__index = DatasetEntity


function DatasetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DatasetEntity)
  self._name = "dataset"
  self._client = client
...
function DatasetEntity:get_name()
  return self._name
end


function DatasetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DatasetEntity.new(self._client, opts)
end
...
function DatasetEntity:mark_deleted()
  self._deleted = true
end


function DatasetEntity:deleted()
  return true == self._deleted
end


function DatasetEntity:data_set(args)
  if args ~= nil then
...
function DatasetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DatasetEntity:match_set(args)
  if args ~= nil then
...
function DatasetEntity:match_get()
...
function DatasetEntity:stream(action, args, callopts)
...
function DatasetEntity:_run_op(ctx, post_done)
  local utility = self._utility
