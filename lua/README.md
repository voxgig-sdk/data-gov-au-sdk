...
The Lua SDK for the DataGovAu API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Dataset(nil):load({ id = "example_id" }, nil)
...
local result, err = client:DataGovAu(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── data-gov-au_sdk.lua    -- Main SDK module
...
The main module (`data-gov-au_sdk`) exports the SDK constructor
