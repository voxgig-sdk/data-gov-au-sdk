...
local DataGovAuSDK = {}
DataGovAuSDK.__index = DataGovAuSDK
...
DataGovAuSDK._make_feature = _make_feature


function DataGovAuSDK.new(options)
  local self = setmetatable({}, DataGovAuSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function DataGovAuSDK:options_map()
...
function DataGovAuSDK:get_utility()
  return Utility.copy(self._utility)
end


function DataGovAuSDK:get_root_ctx()
  return self._rootctx
end


function DataGovAuSDK:prepare(fetchargs)
...
function DataGovAuSDK:direct(fetchargs)
...
-- by passing it.
function DataGovAuSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function DataGovAuSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "DataGovAuSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function DataGovAuSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = DataGovAuSDK.new(sdkopts)
  sdk.mode = "test"
