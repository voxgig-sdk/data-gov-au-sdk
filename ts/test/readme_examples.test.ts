...
//     (\`new DataGovAuSDK(...)\` / \`DataGovAuSDK.test(...)\` become
//     \`DataGovAuSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"dataset":{"test01":{"id":"test01"}},"metadata":{"test01":{"id":"test01"}},"organization":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DataGovAuSDK(...)\` and any
// \`DataGovAuSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DataGovAuSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DataGovAuSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
