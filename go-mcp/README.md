...
[MCP](https://modelcontextprotocol.io) server exposing the DataGovAu SDK as
two agent tools — `data-gov-au_list` and `data-gov-au_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/data-gov-au-mcp)
make build

# 2. Provide credentials via the environment
export DATA_GOV_AU_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user data-gov-au \
  -- /absolute/path/to/data-gov-au-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./data-gov-au-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// data-gov-au_list: first page of records
{ "entity": "metadata" }
{ "entity": "metadata", "query": { } }

// data-gov-au_load: one record by id
{ "entity": "dataset", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/data-gov-au-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user data-gov-au \
     -- "$PWD"/dist/*/data-gov-au-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `data-gov-au_list` and `data-gov-au_load` tools now appear
   in new sessions. Ask the agent to *"list metadata using data-gov-au"*
   and it calls `data-gov-au_list` with `{"entity":"metadata"}`.
...
```sh
export DATA_GOV_AU_APIKEY=sk_live_xxx            # API key
export DATA_GOV_AU_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `data-gov-au_list` tool
...
### Call the `data-gov-au_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `data-gov-au_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `data-gov-au_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DATA_GOV_AU_APIKEY` | API key sent with every request. |
| `DATA_GOV_AU_BASE` | Optional override of the API base URL. |
...
```sh
./data-gov-au-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"data-gov-au_load","arguments":{"entity":"dataset","query":{"id":1}}}}'
```
