...
MCP server exposing the DataGovAu SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `data-gov-au_list` | `entity`, optional `query` map | First page of records as JSON |
| `data-gov-au_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `data-gov-au_list` and
`data-gov-au_load` tools become available in new sessions.
