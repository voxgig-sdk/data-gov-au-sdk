// Command DataGovAu-cli is a CLI + REPL for the DataGovAu SDK,
...
const prompt = "data-gov-au"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "dataset metadata organization"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DATA_GOV_AU_APIKEY (api key), DATA_GOV_AU_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DATA_GOV_AU_APIKEY carries the API key and
	// DATA_GOV_AU_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DATA_GOV_AU_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DATA_GOV_AU_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDataGovAuSDK(opts)
