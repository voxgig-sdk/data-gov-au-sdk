...
boru-driven command-line client **and** interactive REPL for the DataGovAu
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/data-gov-au-cli)
make build

# 2. See usage (words, entities, env vars)
./data-gov-au-cli --help

# 3. Provide credentials once, via the environment
export DATA_GOV_AU_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./data-gov-au-cli load 1 dataset            # {id:1} shorthand
./data-gov-au-cli load '{id:1}' dataset       # explicit match map

# 5. Override the API base URL for a single call
DATA_GOV_AU_BASE=https://api.example.com ./data-gov-au-cli load 1 dataset

# 6. No arguments -> interactive REPL
./data-gov-au-cli
data-gov-au> load 1 dataset
data-gov-au> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/data-gov-au-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/data-gov-au-cli load 1 dataset
   ```
...
```sh
./data-gov-au-cli load 1 dataset          # scalar shorthand for {id:1}
./data-gov-au-cli load '{id:1}' dataset     # explicit match map
```
...
./data-gov-au-cli load 1 dataset
```
...
Run with no arguments to open a REPL (prompt `data-gov-au>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `dataset`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/data-gov-au-cli`. |
...
(`./data-gov-au-cli <expr>`) and interactively (the REPL), and expressions compose the
