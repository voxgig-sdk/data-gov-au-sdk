...
class FtCtrl extends DataGovAuControl
{
...
class FtClient
{
    ...
    private DataGovAuUtility $utility;

    public function __construct(DataGovAuUtility $utility, array $options)
    {
    ...
    public function get_utility(): DataGovAuUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DataGovAuContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DataGovAuContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DataGovAuContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DataGovAuConfig::make_feature($name);
    ...
    private static function buildUrl(DataGovAuSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DataGovAuError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DataGovAuResponse($fetched);
        }
    ...
    private function fail(DataGovAuContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DataGovAuContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
