...
class DatasetEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DATA_GOV_AU_TEST_DATASET_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $dataset_ref01_data = null;
        if (count($dataset_ref01_data_raw) > 0) {
...
function dataset_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("DATA_GOV_AU_TEST_DATASET_ENTID");
