...
class DataGovAuSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = DataGovAuHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = DataGovAuHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!DataGovAuFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, DataGovAuFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return DataGovAuUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = DataGovAuHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = DataGovAuHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = DataGovAuHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new DataGovAuError($op . "_allow",
                "DataGovAuSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = DataGovAuHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new DataGovAuError("graphql_error",
                "DataGovAuSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Dataset($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_dataset === null) {
                $this->_dataset = new DatasetEntity($this, null);
            }
            return $this->_dataset;
        }
        return new DatasetEntity($this, $data);
    }
