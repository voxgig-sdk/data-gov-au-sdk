...
func (sdk *DataGovAuSDK) OptionsMap() map[string]any {
...
func (sdk *DataGovAuSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *DataGovAuSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *DataGovAuSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *DataGovAuSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *DataGovAuSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *DataGovAuSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("DataGovAuSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *DataGovAuSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *DataGovAuSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("DataGovAuSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
