...
import (
	...
	omni "github.com/voxgig-sdk/data-gov-au-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DataGovAuError); is {
				out := map[string]any{
					"name":    "DataGovAuError",
...
func omniCtx(arg any, client *sdk.DataGovAuSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DataGovAuSDK); is {
			client = live
