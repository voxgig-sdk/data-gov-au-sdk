...
func TestDatasetEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DATA_GOV_AU_TEST_DATASET_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		datasetRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.dataset")))
...
func datasetBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DATA_GOV_AU_TEST_DATASET_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DATA_GOV_AU_TEST_DATASET_ENTID"])
